...
static Klass* resolve_field_return_klass(const methodHandle& caller, int bci, TRAPS) {
  ...
  LinkResolver::resolve_field_access(result, constants, field_access.index(), caller, Bytecodes::java_code(code), true, CHECK_NULL);
...
JRT_ENTRY_PROF(void, Runtime1, patch_code, Runtime1::patch_code(JavaThread* current, Runtime1::StubID stub_id))
  ...
  if (stub_id == Runtime1::access_field_patching_id) {
    ...
    LinkResolver::resolve_field_access(result, constants, field_access.index(), caller_method, Bytecodes::java_code(code), true, CHECK);
